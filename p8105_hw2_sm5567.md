...
- I had to remove the first row of data due to it containing repeat
  information that is already found in the columns. \*Then I read the
  `amyloid_df` in csv by using the `read.csv` function. Next, I cleaned
  the name by using the `janitor::clean_names()` function.
- To specify the column names, I used `colnames(amyloid_df)` and defined
  “student_id” as “id”. I renamed the `study_id` variable to `id` for
  consistency when merging.
